...
use super::ort_backend::{Batch, OrtBackend, OrtConfig, OrtEP, YOLOTask};
use super::yolo_results::{Bbox, Embedding, Point2, YOLOResult};
...
fn check_font(font: &str) -> rusttype::Font<'static> {
...
impl YOLOv8 {
    pub fn new(config: YOLOv8Config) -> Result<Self> {
        // execution provider
        let ep = OrtEP::Cpu;
        ...
        let nc = engine.nc().unwrap_or_else(|| {
            panic!("Failed to get num_classes!");
        });

        let (nk, nm) = match task {
            YOLOTask::Pose => {
                let nk = engine.nk().unwrap_or_else(|| {
                    panic!("Failed to get num_keypoints!");
                });
                (nk, 0)
            }
            YOLOTask::Segment => {
                let nm = engine.nm().unwrap_or_else(|| {
                    panic!("Failed to get num_masks!");
                });
                (0, nm)
            }
            _ => (0, 0),
    ...
    pub fn run(&mut self, xs: &Vec<DynamicImage>) -> Result<(Vec<YOLOResult>, Vec<String>)> {
        // pre-process
        ...
        // plot and save
        if self.plot {
            files_names = self.plot_and_save(&ys, xs, Some(&SKELETON));
        }
        Ok((ys, files_names))
    }
    ...
    pub fn plot_and_save(
        ...
        for (_idb, (img0, y)) in xs0.iter().zip(ys.iter()).enumerate() {
            ...
            // mkdir and save
            let mut save_dir = PathBuf::from(self.save_dir.as_ref().unwrap_or(&"runs".to_string()));
            if !save_dir.exists() {
                std::fs::create_dir_all(&save_dir).unwrap();
            }
            save_dir.push(gen_time_string("-"));
            let saveout = format!("{}.jpg", save_dir.to_str().unwrap());
            let _ = img.save(saveout.clone());

            files_names.push(saveout);
        }
        files_names
    }
